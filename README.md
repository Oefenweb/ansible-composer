...
* `composer_self_update`: [default: `false`]: Whether to execute `self-update` on every run
...
* `composer_composer_json_map.{n}.dest`: [required]: The remote path of the file to copy (e.g. `{{ ansible_facts['env']['HOME'] }}/.composer/composer.json`)
...
* `composer_composer_json_map.{n}.run_update`: [default: `false`]: Whether to run `update` on every run (**optional**)
  ...
  vars:
    composer_composer_json_map:
      - src: ../../../files/composer/.composer/composer.json
        dest: "{{ ansible_facts['env']['HOME'] }}/.composer/composer.json"
